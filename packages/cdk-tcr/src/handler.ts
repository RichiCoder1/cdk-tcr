...
export class ProviderBuilder {
  ...
  resource<Schema extends z.Schema>(resource: ResourceDefinition<Schema>, options: ResourceOptions<Schema>) {
    const handler = buildResource(resource.schema, options);
    this.resources.set(resource.typeName, handler);
    return this;
  }
...
function buildResource<Schema extends z.Schema>(schema: Schema, options: ResourceOptions<Schema>) {
  return async function customResourceHandler(event: z.infer<typeof BaseRequest>) {
    const propertiesParseResult = schema.safeParse(event.resourceProperties);
    ...
    switch (event.requestType) {
      ...
      case 'Update':
        ...
        const oldProperties = schema.parse(updateEvent.oldResourceProperties);
